...
input_folder = '/path/to/project/variants/plots/plot_scripts/threshold_screenshots/icgc/'
...
class ImageGui:
    ...
    def __init__(self, master, labels, paths):
        ...
        sorting_string = os.path.basename(df.sorted_in_folder[self.index])
        self.sorting_label = tk.Label(frame, text=("var: %s" % (sorting_string)), width=15)
        ...
        self.sorting_label.grid(row=3, column=1, sticky='we') # +2, since progress_label is placed after
                                                                            # and the additional 2 buttons "next im", "prev im"
...
#             paths.append(path).
    ...
    ### Get probabilty threshold variants
    type_thresholds = {"INS": 0.85, "DEL": 0.85, "INV": 0.45, "DUP": 0.25, "TRA": 0.35}
    types = {"INS": [], "DEL": [], "INV": [], "DUP": [], "TRA": []}
    for f in file_names:
        x = f.split("_")
        t = x[1]
        if t == "":
        ...
        if p == "":
            continue
        p = float(p)
        th = type_thresholds[t]
        if th-0.15 <= p <= th+0.15:
            types[t].append(f)

    file_names = []
    for t in types:
        if len(types[t]) > 100:
            while len(types[t]) > 100:
                types[t].pop(random.randrange(len(types[t])))
        file_names = file_names + types[t]
